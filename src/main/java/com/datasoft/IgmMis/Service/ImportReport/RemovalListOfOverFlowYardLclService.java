...
@Service
public class RemovalListOfOverFlowYardLclService {

    @Autowired
    @Qualifier("jdbcTemplateSecondary")
    private JdbcTemplate secondaryDBTemplate;

    public List getRemovalListOfOverFlowYardLcl(String assignDate){
        String sqlQuery="";
        sqlQuery="SELECT cont_no, \n" +
                "(SELECT sparcsn4.ref_bizunit_scoped.name \n" +
                ...
                "WHERE sparcsn4.inv_goods.gkey=tbl.goods ) AS cf, \n" +
                "(SELECT sparcsn4.ref_bizunit_scoped.sms_number \n" +
                "FROM sparcsn4.ref_bizunit_scoped \n" +
                "INNER JOIN sparcsn4.inv_goods ON sparcsn4.inv_goods.consignee_bzu = sparcsn4.ref_bizunit_scoped.gkey \n" +
                "WHERE sparcsn4.inv_goods.gkey=tbl.goods ) AS sms_number,\n" +
                "cont_status, \n" +
                "IFNULL((SELECT SUBSTRING(sparcsn4.srv_event_field_changes.new_value,7) FROM sparcsn4.srv_event INNER JOIN sparcsn4.srv_event_field_changes ON sparcsn4.srv_event_field_changes.event_gkey=sparcsn4.srv_event.gkey WHERE sparcsn4.srv_event.applied_to_gkey=tbl.gkey AND sparcsn4.srv_event.event_type_gkey IN(18,13,16) AND sparcsn4.srv_event_field_changes.new_value IS NOT NULL AND sparcsn4.srv_event_field_changes.new_value !='' AND sparcsn4.srv_event_field_changes.new_value !='Y-CGP-.' AND sparcsn4.srv_event.gkey < (SELECT sparcsn4.srv_event.gkey FROM sparcsn4.srv_event INNER JOIN sparcsn4.srv_event_field_changes ON sparcsn4.srv_event_field_changes.event_gkey=sparcsn4.srv_event.gkey WHERE sparcsn4.srv_event.event_type_gkey=4 AND sparcsn4.srv_event.applied_to_gkey=tbl.gkey AND metafield_id='unitFlexString01' AND new_value IS NOT NULL ORDER BY sparcsn4.srv_event_field_changes.gkey DESC LIMIT 1) ORDER BY sparcsn4.srv_event.gkey DESC LIMIT 1),(SELECT SUBSTRING(sparcsn4.srv_event_field_changes.new_value,7) FROM sparcsn4.srv_event INNER JOIN sparcsn4.srv_event_field_changes ON sparcsn4.srv_event_field_changes.event_gkey=sparcsn4.srv_event.gkey WHERE sparcsn4.srv_event.applied_to_gkey=tbl.gkey AND sparcsn4.srv_event.event_type_gkey IN(18,13,16) ORDER BY sparcsn4.srv_event_field_changes.gkey DESC LIMIT 1)) AS slot, \n" +
                ...
                "(SELECT sparcsn4.ref_bizunit_scoped.id \n" +
                "FROM sparcsn4.ref_bizunit_scoped \n" +
                "WHERE gkey=tbl.line_op) AS mlo, \n" +
                "mfdch_value,mfdch_desc,seal_nbr1 \n" +
                "FROM \n" +
                "(SELECT inv_unit.id AS cont_no,sparcsn4.inv_unit.gkey,sparcsn4.inv_unit.goods,sparcsn4.inv_unit.line_op, inv_unit.freight_kind AS cont_status,sparcsn4.inv_unit_fcy_visit.flex_string10 AS rot_no,inv_unit.remark,config_metafield_lov.mfdch_value AS mfdch_value, config_metafield_lov.mfdch_desc AS mfdch_desc,seal_nbr1 \n" +
                ...
                "WHERE DATE(inv_unit_fcy_visit.flex_date01)='"+assignDate+"' AND config_metafield_lov.mfdch_value NOT IN ('CANCEL') AND inv_unit.freight_kind='LCL') AS tbl \n" +
                "WHERE remark LIKE 'overflow%'";
        List resultList=secondaryDBTemplate.query(sqlQuery,new RemovalListOfOverFlowYardLclService.RemovalListLcl());
        List listAll = (List) resultList.stream().collect(Collectors.toList());
        return listAll;
    }

    class RemovalListLcl implements RowMapper{
